...
| Cross-run comparison and score trend | Implemented | `compare` (`internal/compare`) diffs two reports by mutant ID into four buckets -- new survivors, fixed survivors (still present, no longer actionable), removed mutants (no longer present at all, any prior verdict), and an unchanged count -- with `--format json` giving all four as clean, always-present fields for CI. `record`/`trend` (`internal/history`) keep an NDJSON score-history log. See `docs/limitations.md` limitation 12 for the ID-matching caveat `compare` inherits. |
